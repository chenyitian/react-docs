...
`Parent` 能通过专门的 `this.props.children`  props 读取子级。**`this.props.children` 是一个不透明的数据结构：** 通过 [React.Children 工具类](ref-01-top-level-api.md#react.children) 来操作。
...
也可以传递ReactFragment 对象 来做有 key 的子级。详见[Keyed Fragments](10.6-create-fragment.md)
...
但是，有时候需要做细粒度的性能控制。这种情况下，可以重写 `shouldComponentUpdate()` 方法返回 false 来让 React 跳过对子树的处理。参考 [React reference docs](ref-03-component-specs.md) 了解更多。
